...
### Added
- **Utility-Based Human Goal Arbitration**: Replaced the fixed-priority top-level motivation selector with normalized utility scoring for danger, sleep, hunger, shelter, thermal comfort, and test felling goals. Added commitment/switch-margin hysteresis, immediate visible-threat preemption, stable behavior-tree execution with wander fallback, runtime goal telemetry, and regression tests for scoring and switch guards.
- **Natural Rock Asset Pack (100 Rocks)**: Added `natural_rock_pack` containing 100 distinct procedural low-poly environmental rock assets (`natural_rock_001` through `natural_rock_100`) spanning 10 cm pebbles to 5 m landmark boulders across 11 natural shape families (tiny pebbles/chips/slabs/wedges, weathered slabs, eroded boulders, clustered twin peaks, shelved crags, tall pillars, angular blocks, squat rocks, wedges, landmark monoliths, and landmark crags). Features single-mesh manifold watertight geometry, strict physical dimension-based triangle budgets (6 to 300 triangles), flat shading with neutral stone PBR materials, bottom-center origins, UV unwrap, authoritative `.blend` scene, dual-angle contact sheet visual QC, and automated dual validation in Blender and Unity.
